                                                               ...
                                                               if_else(ind_age %in% 12:17,"12-17",
                                                                       if_else( ind_age %in% 18:24, "18-24",
                                                                                if_else(ind_age %in% 25:59 ,"25-59",
                                                                                        if_else(ind_age >= 60,"60+","",NULL)))))),
    I.HEALTH.age_groups_medical.INDV = if_else(ind_age < 18,"0-17",
                                               if_else(ind_age %in% 18:59,"18-59",
                                                       if_else(ind_age >= 60,"60+","",NULL))),
    
    I.EDU.age_group_education.INDV = if_else(ind_age %in% 5:11,"5-11",
                                             if_else(ind_age %in% 12:17,"12-17",
                                                     if_else(ind_age %in% 18:24,"18-24","",NULL)))
...
# Individual to Household_both_refugee_and_host_community -------------------------------------------------
 ...
 if (population == "Refugee"){ 
  ...
  indiv_to_hh <- individual_data %>% 
    mutate(
      atleast_one_child_hazardous_rowsum = rowSums(atleast_one_child_hazardous),
    ...
    summarise(
      ...
      I.P.safety_issues_barriers_health_facilities.INDVHH = if_else(all(HEALTH.safety_concerns_access_health_sum_explicit_na == "NA"),"NA", 
      ...
      # I.P.safety_issues_barriers_key_facilities_any.HH = if_else(I.P.safety_issues_barriers_key_facilities.INDVHH == "yes" | I.P)
...
# Household to Household  ---------------------------------------------------------------
  ...
  hh_to_hh<- mutate(hh_data,
                    ...
                    I.HH_CHAR.gender_hoh.HH = if_else(respondent_hoh == "yes",respondent_gender, hh_data$hoh_gender, NULL ),
                    
                    I.HH_CHAR.age_hoh.HH = if_else(respondent_hoh == "yes", true =respondent_age, false = hoh_age, missing = NULL ),
                    
                    
                    I.P.married_women_no_work_market_alone.HH = if_else( married_work_mobility == "" & married_market_mobility == "","",
                                                                         if_else(married_work_mobility == "never_alone" & married_market_mobility == "never_alone","yes","no")),
  ...
  if (population == "Refugee"){
    ...
    safety_issues_barriers_key_facilities_included_education <- hh_data[,c("education_barrier.center_unsafe", 
                                                                      "education_barrier.way_unsafe")]
    safety_issues_barriers_key_facilities_included_market <- hh_data[,c("market_problems.market_unsafe_way", 
                                                                   "market_problems.market_unsafe")]
    
    P.awareness_community_protection_atleast_two <- hh_data[,c("comm_mechanisms.health", 
      ...
      mutate(
        ...
        I.HH_CHAR.datearrival_bgd.HH = if_else(anydate(datearrival_bgd) < dmy("01/01/2016")  , true = "pre_2016",
                                               if_else(anydate(datearrival_bgd) %in%  dmy("01/01/2016"):dmy("31/07/2017"),"Jan 2016- July 2017",
                                                       if_else(anydate(datearrival_bgd) > dmy("31/07/2017")  , true = "after_july_2017", false = " ", missing = NULL))),
        ...
        I.SNFI.pay_shelter_materials_improvements.HH= if_else(shelter_purchased == "yes" | exp_shelter_materials %in% exp_shelter_materials_greater_than_zero, "yes","no",NULL),
        ...
        I.P.safety_issues_barriers_markets.HH = if_else(safety_issues_barriers_key_facilities_included_market_rowsum >=1 , "Market_unsafe","no" ,NULL),
        ...
        # I.P.safety_issues_barriers_key_facilities_any.HH = if_else(I.P.safety_issues_barriers_key_facilities.HH_education == "Education_unsafe" |
        #                                                              I.P.safety_issues_barriers_key_facilities.HH_market == "Market_unsafe" |
        #                                                              indv_to_hh$I.P.safety_issues_barriers_key_facilities.INDVHH == "Health_unsafe","yes","no")
        # 
...
# individual to individual ------------------------------------------------
    ...
    Ind_to_Ind_final <- mutate( ind_to_ind,
                          ...
                          I.HEALTH.ind_smoke_some_all_days.INDV = if_else(ind_age <12, "",
                                                                            if_else( ind_smoke == "everyday" |ind_smoke == "some_days", "yes", "no",NULL)),
                          I.EDU.age_group_education_refugee.INDV = if_else(ind_age %in% 3:4,"3-4","",NULL)
                          
                          
                          
                          
    ) %>%  select(X_submission__uuid,starts_with("I."))
  ...
  if (population == "Host") {
    ...
    hh_to_hh <- hh_with_individual_level_data %>% mutate(
      ...
      FSL.host_community_food_assistance_only_excluded_rowsum = rowSums(FSL.host_community_food_assistance_only_excluded),
      FSL.host_community_food_assistance_only_included_rowsum = rowSums(FSL.host_community_food_assistance_only_included),
...
Ind_to_Ind_final <- mutate(ind_to_ind,
                      ...
                      I.EDU.formal_2018_not_2019.INDV = if_else( ind_formal_learning_prev %in% formal_education & ind_formal_learning == "none", "yes","no",NULL),
                      ...
                      I.EDU.age_group_education_HC.INDV = if_else(ind_age == 4,"4","",NULL)
